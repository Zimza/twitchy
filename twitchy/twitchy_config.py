...
class ConfigInit:
    ...
    def configure_options(self):
        try:
            ...
            self.default_quality = input(
                " Default stream quality [low/medium/HIGH/super/source]: "
            )
            if self.default_quality == "" or self.default_quality not in [
                "low",
                "medium",
                "super",
                "source",
            ...
            try:
                self.truncate_status_at = int(
                    input(" Truncate stream status at [AUTO]: ")
                )
            except ValueError:
            ...
            try:
                final_decision = input(
                    Colors.RED + " Do you wish to restart? [y/N]: " + Colors.ENDC
                )
...
class Options:
    ...
    def parse_options(self):
        ...
        display_scheme = non_interactive.get(
            "DisplayScheme",
            ("GameName", "GameAltName", "ChannelName", "ChannelAltName"),
        )
        ...
        try:
            ...
            Video = collections.namedtuple("Video", ["default_quality", "player_final"])
            self.video = Video(default_quality, player_final)
            ...
            Colors = collections.namedtuple(
                "Colors", ["numbers", "game_name", "column1", "column2", "column3"]
            )
            self.colors = Colors(
            ...
            Chat = collections.namedtuple("Chat", ["enable"])
            self.chat = Chat(chat_section.getboolean("Enable", True))
